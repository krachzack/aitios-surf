use geom::{InnerSpace, Normal, Position, Vec3};
...
impl<S: Position> Surface<S> {
    ...
    pub fn nearest_mut<'a>(&'a mut self, from: Vec3) -> &'a mut S {
        ...
        let (_, &nearest_idx) = self
            .spatial_idx
            .nearest(&[x, y, z], 1, &squared_euclidean)
            .unwrap()[0];
    ...
    pub fn nearest_n<'a>(&'a self, from: Vec3, count: usize) -> Vec<(f32, &'a S)> {
        ...
        let nearest_idxs = self
            .spatial_idx
            .nearest(&[x, y, z], count, &squared_euclidean)
            .unwrap();
    ...
    pub fn nearest_n_indexes_oriented<'a>(
        &'a self,
        from: Vec3,
        normal: Vec3,
        ...
        S: Normal,
    {
        ...
        self.spatial_idx
            .iter_nearest(&[x, y, z], &squared_euclidean)
            .unwrap()
            .filter(|n| {
    ...
    pub fn find_within_sphere<'a>(&'a self, center: Vec3, radius: f32) -> Vec<&'a S> {
        ...
        self.spatial_idx
            ...
            .unwrap()
            .iter()
            .map(move |&(_, &idx)| &self.samples[idx])
            .collect()
    }
...
mod test {
    use super::*;
    use geom::{FromVertices, TupleTriangle, Vec2, Vertex};
    use SurfaceBuilder;
    use SurfelSampling;
    ...
    fn unit_quad() -> Vec<TupleTriangle<Vertex>> {
        ...
        vec![
            TupleTriangle::new(
                vtx_bottom_left.clone(),
                vtx_bottom_right,
                vtx_top_right.clone(),
            ),
            TupleTriangle::new(vtx_top_right, vtx_top_left, vtx_bottom_left),
        ]
    }
}
